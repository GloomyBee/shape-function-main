...
Files no longer generated by default:

- `metrics.json`
- `summary.txt`
- `best_model.pt`

`checkpoint.pt` is the only formal checkpoint artifact. It stores the best weights together with run metadata.

## Visualization Quick Start

The repository now uses a single formal visualization entrypoint:
...
Recommended figure views for paper inspection:
...
Supplementary views remain available:
